...
public class LuckMinigame : MonoBehaviour
{
    ...
    [Header("Direct Input")]
    [SerializeField] private Camera luckCamera;
        ...
        bool ai)
    {
        ...
        {
            Debug.LogError(
                "LuckMinigame: Assign enough hats and hat positions."
            );
    ...
    private void HandleDirectMouseInput()
    {
        ...
        {
            Debug.LogError(
                "LuckMinigame: No camera assigned and Camera.main is null."
            );
        ...
        LuckHat clickedHat =
            hit.collider.GetComponentInParent<LuckHat>();

        if (clickedHat == null)
            return;

        int clickedIndex =
            GetHatIndex(clickedHat);
        ...
        int visibleHatIndex)
    {
        ...
        if (
            visibleHatIndex < 0 ||
            visibleHatIndex >= activeHatCount)
        {
            return;
        }
    ...
    private IEnumerator FinishAfterReveal(
        bool won)
    {
        yield return new WaitForSeconds(
            0.8f
